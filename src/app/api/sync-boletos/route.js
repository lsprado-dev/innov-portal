...
export async function GET(request) {
  ...
  try {
    ...
    // MÁGICA 1: OTIMIZADA E BLINDADA (15 dias antes do início do mês atual)
    const dataInicioBase = new Date(dataLocal.getFullYear(), dataLocal.getMonth(), 1);
    dataInicioBase.setDate(dataInicioBase.getDate() - 15); // Recua 15 dias do dia 1º
    const strIniBase = dataInicioBase.toISOString().split('T')[0];
    const strFimBase = dataLocal.toISOString().split('T')[0];

    // MÁGICA 2: Janela do Futuro (O início acompanha a base para barrar lixo)
    const strIniFuturo = strIniBase;
